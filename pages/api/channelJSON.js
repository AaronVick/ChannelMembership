...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Fetch all channels and cache the result
...
// Fetch all channels for FID with async handling and proper pagination
async function fetchAllChannelsForFid(fid) {
  ...
  try {
    do {
      ...
      // Log the fetched channels for debugging
      console.log(`Fetched ${channels.length} channels. Total so far: ${allChannels.length}`);
...
// Render HTML for all channels including JSON dump
function renderHTML(channels, fid, channelName, res) {
  const channelListHTML = channels.map((channel, index) => {
    ...
    // All channel fields displayed, plus raw JSON dump
    return `
      <div style="background-color: ${alternatingBgColor}; padding: 10px; margin-bottom: 5px;">
        ...
        <pre>${JSON.stringify(channel, null, 2)}</pre> <!-- Display full JSON for each channel -->
      </div>
    `;
  }).join('');
